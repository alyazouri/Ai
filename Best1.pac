...
//
//   Version:  8.0 - Final Clean
//
//   🏠 اللوبي     → SOCKS5 46.185.131.218:20003
...
var P   = "46.185.131.218";
var LOB = "SOCKS5 " + P + ":20003";   // لوبي (تجنيد أردنيين)
var MAT = "SOCKS5 " + P + ":20001";   // مباريات (سيرفر محدد)
var DIR = "DIRECT";                    // مباشر (بنق أخفض)
...
function FindProxyForURL(url, host) {
    ...
    // ─── ① الصوت → DIRECT ───────────────────────
    if (dM(h, D_VOICE)) return DIR;

    // ─── ② الحماية → DIRECT ─────────────────────
    if (dM(h, D_SAFE)) return DIR;

    // ─── ③ التقارير → DIRECT ────────────────────
    if (dM(h, D_REPORT)) return DIR;

    // ─── ⑤ المباريات (سيرفرات IP) → 20001 ──────
    if (isMS(host)) return MAT;

    // ─── ⑤ المباريات (دومينات) → 20001 ──────────
    if (dM(h, D_MATCH)) return MAT;

    // ─── ⑥ اللوبي والتجنيد → 20003 ──────────────
    if (dM(h, D_LOBBY)) return LOB;

    // ─── ⑦ الدومين الرئيسي → 20003 ──────────────
    if (dM(h, D_MAIN)) return LOB;
